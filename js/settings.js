...
async function getLanguages() {
  ...
  const mozillaLangs = Array.from(sitemap.querySelectorAll('loc'))
    .map((node) => re.exec(node.textContent))
    .filter((match) => match) // remove null matches
    .map(([, lang]) => lang); // match object is [fullText, matchGroup]

  const languagesJoin = mozillaLangs.map((lang) => {
    ...
    if (lang in ianaLangs) {
      ...
      const [base, subScript] = lang.split('-');
...
function recurseSave(nodes) {
  const check = Array.from(nodes).filter((i) => i.tagName === 'INPUT')[0];
...
function createElementFromMap(obj, parentName, depth) {
  ...
  if (entries.length > 1 && depth < 3) {
    const mapOut = entries.map(([key, child]) => {
  ...
  // Alter children
  const initialCheckChildNodes = initalCheckbox.parentNode.querySelectorAll('input[type="checkbox"]');
  initialCheckChildNodes.forEach((node) => { node.checked = initalCheckbox.checked; });
...
async function renderTree() {
  ...
  const { matchSlugs } = matchSlugsWrapper;
...
async function saveRequeryUrl() {
  const currentLang = document.querySelector('select#langs').value;
